...
async function authorize(db,env,input,playerId,identity) {
  ...
  const user=await findUser(db,identity.sub);const player=await findPlayer(db,playerId);
  if(!user||!user.is_active||user.role!=='PLAYER')throw new RegistrationError('Player authentication required',403,'FORBIDDEN');
  if(player?.user_id!==user.id)throw new RegistrationError('User does not own this player profile',403,'FORBIDDEN');
  if(input.userId!==undefined&&input.userId!==user.id)throw new RegistrationError('Cannot act for another user',403,'FORBIDDEN');
}
export const previewEligibility=(env,input)=>withTransaction(env,db=>checkEligibility(db,input));
export async function createRegistration(env,input,identity){
  if(!identity)throw new RegistrationError('Authentication required',401,'UNAUTHENTICATED');
  input=validateRegistrationInput(input);return withTransaction(env,async db=>{
  await authorize(db,env,input,input.playerId,identity);await repo.lockWrites(db);
...
export function cancelRegistration(env,id,input={},identity) {return withTransaction(env,async db=>{
  ...
  await authorize(db,env,input,row.player_id,identity);
