...
export async function handleRegistrationRoutes(request,env){try{
  ...
  if(!parts.length){
    ...
    if(request.method==='POST')return successResponse(await service.createRegistration(env,await body(request)),201);
    ...
    if(request.method==='POST')return successResponse(await service.cancelRegistration(env,parts[0],await body(request,true)));
