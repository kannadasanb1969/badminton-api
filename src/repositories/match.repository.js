import {completedKnockoutFinals} from './completion.repository.js';
export async function all(db, q={}){return (await db.query('SELECT * FROM matches WHERE ($1::text IS NULL OR fixture_id=$1) AND ($2::text IS NULL OR tournament_id=$2) ORDER BY round_number,match_number',[q.fixtureId??null,q.tournamentId??null])).rows;}
export async function byId(db,id,lock=false){return (await db.query(`SELECT * FROM matches WHERE id=$1${lock?' FOR UPDATE':''}`,[id])).rows[0];}
export async function fixture(db,id){return (await db.query('SELECT f.status,f.format,t.organizer_id FROM fixtures f JOIN tournaments t ON t.id=f.tournament_id WHERE f.id=$1',[id])).rows[0];}
export async function participantDisplay(db,id,type){if(type==='PLAYER')return (await db.query('SELECT full_name AS name,player_code AS code FROM player_profiles WHERE id=$1',[id])).rows[0];if(type==='TEAM'){const t=(await db.query('SELECT * FROM teams WHERE id=$1',[id])).rows[0];if(!t)return null;const a=t.player1_type==='GUEST'?'guest_players':'player_profiles',b=t.player2_type==='GUEST'?'guest_players':'player_profiles';const ca=t.player1_type==='GUEST'?'guest_code':'player_code',cb=t.player2_type==='GUEST'?'guest_code':'player_code';const [p1,p2]=await Promise.all([db.query(`SELECT full_name AS name,${ca} AS code FROM ${a} WHERE id=$1`,[t.player1_id]),db.query(`SELECT full_name AS name,${cb} AS code FROM ${b} WHERE id=$1`,[t.player2_id])]);return {name:[p1.rows[0]?.name,p2.rows[0]?.name].filter(Boolean).join(' / '),code:t.team_code};}return null;}
export async function history(db,id){return (await db.query('SELECT * FROM match_score_history WHERE match_id=$1 ORDER BY created_at,id',[id])).rows;}
export async function updateStart(db,id,points){return (await db.query("UPDATE matches SET status='LIVE',winning_points=$2,started_at=COALESCE(started_at,NOW()),updated_at=NOW() WHERE id=$1 RETURNING *",[id,points])).rows[0];}
export async function updateScore(db,id,a,b,status,winner){return (await db.query("UPDATE matches SET participant1_score=$2,participant2_score=$3,status=$4::varchar,winner_id=$5,completed_at=CASE WHEN $4::varchar='COMPLETED' THEN NOW() ELSE completed_at END,updated_at=NOW() WHERE id=$1 RETURNING *",[id,a,b,status,winner])).rows[0];}
export async function addHistory(db,id,a,b,action,user){return (await db.query('INSERT INTO match_score_history (match_id,participant1_score,participant2_score,action,changed_by) VALUES ($1,$2,$3,$4,$5) RETURNING *',[id,a,b,action,user])).rows[0];}

export async function isFinalMatch(db,match){return (await completedKnockoutFinals(db,[match.tournament_id])).some(final=>final.id===match.id);}
export async function advanceWinner(db,sourceId,winnerId,winnerType) {
  const source=(await db.query('SELECT next_match_id,next_match_slot FROM matches WHERE id=$1 FOR UPDATE',[sourceId])).rows[0];
  if(!source?.next_match_id)return null;
  const column=source.next_match_slot===1?'participant1_id':'participant2_id';
  const typeColumn=source.next_match_slot===1?'participant1_type':'participant2_type';
  return (await db.query(`UPDATE matches SET ${column}=$2,${typeColumn}=$3,updated_at=NOW() WHERE id=$1 AND status='SCHEDULED' RETURNING *`,[source.next_match_id,winnerId,winnerType])).rows[0];
}
